...
### 2.4 客户搜索正式支持拼音索引
- **现状**：移动端已先做轻量版前端拼音/首字母过滤，适合客户量较小时使用
- **后续**：后端客户表增加 `name_pinyin`、`name_initials` 字段，并建立索引
- **搜索**：客户搜索同时匹配姓名、电话、标签、`name_pinyin`、`name_initials`
- **收益**：客户量变大后，输入 `zhou`、`z` 等拼音/首字母仍可稳定快速搜索
